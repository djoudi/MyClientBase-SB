<dl>
	...
	<dd>
		...
		<select name="tax_rate_id[]">
			...
			<option value="<?php echo $tax_rate->tax_rate_id; ?>" <?php if($invoice_tax_rate->tax_rate_id == $tax_rate->tax_rate_id) { ?>selected="selected"<?php } ?>><?php echo format_number($tax_rate->tax_rate_percent, TRUE, $this->mcbsb->settings->setting('decimal_taxes_num')) . '% - ' . $tax_rate->tax_rate_name; ?></option>
			<?php } ?>
		</select>
...
<input class="uibutton" style="float: right; margin-top: 10px; margin-right: 10px;" type="submit" id="btn_submit" name="btn_submit_options_tax" value="<?php echo $this->lang->line('save_options'); ?>" /></dd>
