...
class Personal_Settings extends Admin_Controller {
	...
	private function get_settings_tabs($module, $settings_view, $tab_name = null){
		...
		return array(
				...
				'settings_view'	=>	$module->module_config['personal_settings_view'],
				'html'			=>  $html
		);
	...
	public function index() {
		...
		foreach ($this->mcbsb->_modules['enabled'] as $item) {
			...
			if($module->read()){
				...
				if (!empty($module->module_config['personal_settings_view']) and isset($module->module_config['personal_settings_save'])) {
					
					$settings_views = $module->module_config['personal_settings_view'];
		...
		$this->load->view('personal_settings.tpl', $data, false, 'smarty');
	}
